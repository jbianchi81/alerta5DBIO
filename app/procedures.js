...
internal.runProcedureTests = async function(procedure, result) {
    if(!procedure.tests || !procedure.tests.length) {
        console.warn("No tests to run")
        return
    }
    for(const test of procedure.tests) {
        const test_results = await test.run(result)

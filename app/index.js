...
program
	...
	.option('-m, --print_masked_color_map', 'Imprime mapas rgb 3horarios y diarios con máscara cdp')
	.option('-t, --time <value>', 'analysis time (hour), default=6', 6)
	.option('-D, --dont_download', 'No descargar')
	.action(async options => {
		try {
			if (options.dont_download) {
				const result = await global.pool.query("WITH maxfd as (\
			...
			console.log("result:" + observaciones.length + " rows found/upserted")
			if (options.print_color_map) {
				var timeupdate = observaciones[0].timeupdate
				const serie = await crud.getSerie("rast", options.series_id, undefined, undefined, { format: "gtiff" }, undefined, timeupdate)
				for (var i = 0; i < serie.observaciones.length; i++) {
					...
					var parameters = { title: "Pronostico GFS-SMN [mm/3h]", filename: filename } // {...options}
					parameters.print_color_map = (options.print_color_map) ? options.print_color_map : false
					//~ result[i].valor = '\\x' + result[i].valor
					await print_rast(parameters, serie, serie.observaciones[i])
				}
				console.log("3h maps generados")
			}
			if (options.output) {
				fs.writeFileSync(options.output, JSON.stringify(observaciones, null, 2))
			...
			var timestart = observaciones[0].timestart
			var timeend = observaciones[0].timeend
			var timeupdate = observaciones[0].timeupdate
			for (var i = 0; i < observaciones.length; i++) {
				if (observaciones[i].timestart < timestart) {
					timestart = observaciones[i].timestart
				}
				if (observaciones[i].timeend > timeend) {
					timeend = observaciones[i].timeend
				}
				if (observaciones[i].timeupdate < timeupdate) {
					timeupdate = observaciones[i].timeupdate
				}
			}
			console.log({ timestart: timestart, timeend: timeend })
			const serie_regular = await crud.getRegularSeries("rast", options.series_id, "1 day", timestart, timeend, { insertSeriesId: options.series_id_diario, timeupdate: timeupdate, t_offset: { hours: 9 } })   // options: t_offset,aggFunction,inst,timeSupport,precision,min_time_fraction    ,t_offset:{"hours":9}
			var timestart = serie_regular[0].timestart
			var timeend = serie_regular[0].timeend
			for (var i = 0; i < serie_regular.length; i++) {
				if (serie_regular[i].timestart < timestart) {
					timestart = serie_regular[i].timestart
				}
				if (serie_regular[i].timeend > timeend) {
					timeend = serie_regular[i].timeend
				}
			}
			const serie_diaria = await crud.getSerie("rast", options.series_id_diario, timestart.toISOString(), timeend.toISOString(), { format: "gtiff" })
			for (var i = 0; i < serie_diaria.observaciones.length; i++) {
				...
				var filename = location + "/" + "gfs." + serie_diaria.observaciones[i].timeupdate.toISOString().replace(/[-T]/g, "").substring(0, 10) + "." + serie_diaria.observaciones[i].timestart.toISOString().replace(/[-T]/g, "").substring(0, 10) + ".diario.GTiff"
				var parameters = { title: "Pronostico GFS-SMN [mm/d]", filename: filename } // {...options}
				parameters.print_color_map = (options.print_color_map) ? options.print_color_map : false
				parameters.print_masked_color_map = (options.print_masked_color_map) ? options.print_masked_color_map : false

				//~ result[i].valor = '\\x' + result[i].valor
				await print_rast(parameters, serie_diaria, serie_diaria.observaciones[i])
			}
			...
			const serie = await crud.rastExtract(options.series_id_diario, timestart, timeend, { format: "GTiff", "function": "SUM" }) // timestart.toISOString(),timeend.toISOString(),{format:"GTiff", "function":"SUM"})
			if (!serie) {
			...
			await print_rast(
				{ 
					...
					print_masked_color_map: true, 
					filename: filename, 
					title: "Pronostico a 7 dias GFS(SMN) [mm]" 
				}, 
				serie, 
				serie.observaciones[0]
			)			
			const series_areales = await Promise.all([
			...
			console.log("Inserted " + ((series_areales[1]) ? series_areales[1].length : 0) + " registros areales diarios")
			console.log("Inserted " + ((series_areales[0]) ? series_areales[0].length : 0) + " registros areales 3horarios")
			await print_output(options, series_areales)
		} catch(e) {
			console.error(e)
		}
		finally {
		...
		.then(() => {
			...
			if (options.format == "GTiff") {
				...
				if (options.print_color_map) {  // imprime png de grass a partir del gtiff generado
					console.debug("Imprimiendo mapa rgb")
				...
				if (options.print_masked_color_map) {  // imprime png de grass a partir del gtiff generado
					console.debug("Imprimiendo mapa rgb con máscara")
					if (global.config.grass) {
						Object.keys(global.config.grass).map(key => {
							options[key] = (options[key]) ? options[key] : global.config.grass[key]
						})
					...
					//~ console.log(options)
					var output_filename = filename.replace(/(\.GTiff)?$/, ".mask.png")
					var parameters = { ...options }
					parameters.mask_cdp = true
					parameters.timestart = obs.timestart
        			parameters.timeend = obs.timeend 
				    parameters.title = options.title || serie.fuente.nombre
					promises.push(
						printMap.print_pp_cdp_semanal(
							filename, 
							output_filename, 
							parameters
						)

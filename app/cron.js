...
const logfile = path.resolve(config.cronjobs.logfile)
...
const logger = winston.loggers.add('cronjob-logger',{
    transports: [
        ...
        new winston.transports.File({
            filename:config.cronjobs.logfile,
            format: myLogFormat
        })
    ]
})
...
var transporter = nodemailer.createTransport({
    service: 'gmail',
    auth: {
      user: config.cronjobs.email.address,
      pass: config.cronjobs.email.password
    }
});
...
const cronjob = class {
    constructor(name,cron_expression,command,options,alert) {
        this.alert = alert
        this.name = name
        ...
        this.logfile = (options.logfile) ? path.resolve(options.logfile) : logfile
        this.schedule()
    }
    schedule() {
...
internal.my_tasks = [
    {
        ...
        "options": {
            "scheduled": false,
            "logfile": "cron/log/runAsocACUMAR.log"
        },
        "alert": true,
        "command": "node cron/runAsocACUMAR run"
    },
    // {
    //     "name": "deleteCorridas",
    //     "cron_expression": "0 5 * * *",
    ...
    //             }
    //             return pool.query("refresh materialized view series_stats_25")
    //         })
    //     }
    // },
    // {
...
for (var task of internal.my_tasks) {
    ...
    internal.scheduled_tasks.push(new cronjob(task.name,task.cron_expression,task.command,task.options,task.alert))
}

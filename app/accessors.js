...
internal.Accessor = class {
	...
	async getData(series_id,timestart,timeend) {
		...
		return this.engine.get({series_id:series_id,timestart:timestart,timeend:timeend})
	}

	async updateData(series_id,timestart,timeend) {
		// get observaciones from one series between timestart and timeend and update
		return this.getData(series_id,timestart,timeend)
		.then(observaciones=>{
	...
	async getMetadata(filter={}) {
		if(!this.engine.getSeries) {
			throw("getSeries not implemented for accessor " + this.clase)
		}
		return this.engine.getSeries(filter)
	}

	async updateMetadata(filter={},options={}) {
		...
		if(this.engine.updateSeries) {
			return this.engine.updateSeries(filter) //,{all:true})
		} else if(this.engine.getSeries) {
			return this.engine.getSeries(filter)
			.then(series=>{
				const result = crud.upsertSeries(series,false,true)
	...
	async getSeries(filter={},options={}) {
		...
		if(!this.engine.getSeries) {
			var observaciones = await this.engine.get(filter,options)
		...
		const series = await this.engine.getSeries(series_filter,options)
		const results = []
		for(var i in series) {
			if(!series[i].id) {
	...
	async updateSeries(filter={},options={}) {
		...
		for(var i in series) {
			...
			if(this.engine.updateSerie) {
				...
				// console.log(`Updating serie ${series[i].id}`)
				try {
					series[i].setObservaciones(await this.engine.update({series_id:series[i].id,tipo:series[i].tipo, timestart:filter.timestart,timeend:filter.timeend,forecast_date: filter.forecast_date}))
					results.push(series[i])
				} catch (e) {
					console.error(e)
				}
...
internal.ana = class {
		...
		.then(estaciones=>{
		  var o = (async ()=>{
			...
			for(var i=0;i<estaciones.length;i++) {
				...
				var series = await crud.getSeries('puntual',{estacion_id:e.id,proc_id:1,var_id:filter.var_id})
	...
	async getSeries(filter={}) {
		const estaciones = await this.getSitesANA(this.config.sites_local_file,true,0,0)
		const variables = [
	...
	async getSitesANA(file,download=false,statusEstacoes=0,origem=1,format="json") {
		...
		result.DataSet["diffgr:diffgram"][0].Estacoes[0].Table.forEach(async e=>{
			...
			await estacion.getEstacionId(global.pool)
...
internal.mch_py = class {
	...
	getSeries(filter,options={}) {
		const series = []
		return this.getSeriesPage(this.config.url + "/stations",filter,series,(options.return_raw) ? true : false)
	}
	...
	updateSeries(filter,options={}) {
		...
		.then(series=>{
			return crud.upsertSeries(series,options.all)		
		})
	}
...
internal.sat2 = class {
	...
	async updateSeries(filter,options={}) {
		...
		.then(series=>{
			return crud.upsertSeries(series,false,true) // options.all)
		})
	}
	async getSites(filter) {  // filter.id_externo = idEquipo, filter.nombre =~ descripcion
		...
		.then(async estaciones=>{
			...
			for(var estacion of estaciones) {
				...
				await estacion_a5.getEstacionId(global.pool)
				var series = []
	...
	updateSites(filter,options={}) {
		...
		.then(async estaciones=>{
			...
			for(var estacion of estaciones) { //for(var i=0;i<estaciones.length;i++) {
				...
				try {
					var estacion_a5 = await crud.upsertEstacion(estacion)
				} catch (e) {
					console.error(e)
					continue
				}
				if(estacion_a5) {
					if(options.update_series) {
						...
						for(var serie of estacion.series) {
							...
							try {
								serie = await crud.upsertSerie(new CRUD.serie(serie))
							} catch(e) {
								console.error(e)
								continue
							}

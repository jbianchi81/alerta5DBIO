...
test("crea grupo, asigna usuario y red", async ()=> {
  ...
  const res2 = await request(app)
    .put(`/groups/test_create/members`)
    .send([
        {
            user_id: 5
        },{
...
test("GET /obs/puntual/series", async () => {
  const res = await request(app)
    .get("/obs/puntual/series?tabla=alturas_prefe&id_externo=t665a443&var_id=1&proc_id=1&unit_id=11")
    .set("Authorization", `Bearer ${token}`);
...
test("PUT /obs/puntual/serie/{id}?", async () => {
  const res2 = await request(app)
    .put(`/obs/puntual/series/${serie.id}?var_id=27`)
    .set("Authorization", `Bearer ${token}`);

  assert.equal(res2.statusCode, 200);
  assert("id" in res2.body)
  assert.equal(res2.body.id,serie.id)
  assert("var" in res2.body)
  assert("id" in res2.body.var)
  assert.equal(res2.body.var.id,27)
});
...
test("PUT /obs/puntual/series/{id}? no access", async () => {
  const res2 = await request(app)
    .put(`/obs/puntual/series/${serie.id}?var_id=31`)
    .set("Authorization", `Bearer token_1`);

  assert.equal(res2.statusCode, 401);
});

test("DELETE /obs/puntual/series/estaciones/{id} no access", async () => {
  const res = await request(app)
...
test("DELETE /obs/puntual/series/{id}", async () => {
  ...
  assert.ok("tabla" in serie_.estacion)
  assert.equal(serie_.estacion.tabla,"alturas_prefe")
  assert.ok("id_externo" in serie_.estacion)
  assert.equal(serie_.estacion.id_externo,"t665a443")
})